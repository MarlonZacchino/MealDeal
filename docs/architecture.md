...
desselben MealPlanEntry liefert denselben persistierten History-Eintrag. Der Service berührt
bewusst weder Planung noch Inventory. `RecipeFeedbackService` verwaltet den einen aktuellen
...
`IngredientManagementService` kapselt das Anlegen, Umbenennen und Umkategorisieren zentraler Ingredients. Er validiert nichtleere, ohne Beachtung der Groß-/Kleinschreibung eindeutige Namen und löst die gewählte Kategorie aus dem zentralen Katalog auf. Beim Aktualisieren speichert er ein neues unveränderliches `Ingredient`-Objekt mit derselben UUID; neu angelegte Zutaten erhalten ihre eigene stabile UUID. Da Recipes und InventoryItems über diese UUID referenzieren, laden beide Bereiche anschließend automatisch Name und Kategorie in ihrem aktuellen Stand; JavaFX-Controller enthalten dafür keine Persistenz- oder Referenzlogik.
