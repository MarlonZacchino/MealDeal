...
Die Zutaten-Suche lädt zentrale Ingredients und Recipes über ihre Repository-Schnittstellen. `IngredientSearchModel` hält ausschließlich den JavaFX-unabhängigen Auswahlzustand von einer bis zehn Zutaten und reicht die geladenen Recipes und ausgewählten Ingredient-Identitäten unverändert an `RecipeSearchService.searchByIngredients()` weiter. Ranking, Trefferzahl, fehlende Zutaten und Qualitätsstufe stammen ausschließlich aus dessen `IngredientSearchResult`; die UI übernimmt die bestehende Reihenfolge und navigiert vom Ergebnis zum vollständigen Recipe-Detail.

Die Geschmackssuche folgt derselben Trennung: `TasteSearchModel` lädt zentrale Tastes, verwaltet nur deren Auswahlzustand und delegiert `AND`, `OR` oder `RANKING` unverändert an `RecipeSearchService.searchByTastes()`. Zutaten- und Geschmackssuche besitzen getrennte Aktionen und werden nicht kombiniert; beide teilen lediglich den Ergebnisbereich und die bestehende Navigation zur Recipe-Detailansicht.
