...
`WeeklyMealPlanService` bildet darauf den Anwendungsfall für die aktuelle Woche. Ein injizierbarer `Clock` bestimmt das lokale Heute, `WeekService` liefert die sieben konkreten Datumswerte und `MealPlanRepository.findBetween()` lädt vorhandene Planungen. Für jeden Tag entsteht ein unveränderlicher `MealPlanDay` mit optionalem Eintrag und Heute-Markierung. Planen und Ersetzen verwenden unverändert `MealPlanRepository.save()`; die eindeutige Datumsspalte garantiert weiterhin höchstens einen Eintrag. Entfernen löst den vorhandenen Eintrag über das Repository auf und löscht ausschließlich dessen UUID. Die individuelle Personenanzahl bleibt Bestandteil jedes `MealPlanEntry`.
...
`CreateRecipeController` hält ausschließlich den dynamischen JavaFX-Formularzustand und die Navigation. Derselbe FXML- und Controller-Stack dient dem Erstellen und Bearbeiten: Im Bearbeitungsmodus werden alle vorhandenen Werte vorausgefüllt, Abbrechen führt zum unveränderten Detailobjekt zurück und Speichern zeigt das aktualisierte Detailobjekt. `RecipeFormService` verarbeitet einen JavaFX-unabhängigen `RecipeFormInput`, validiert ihn vollständig, löst bestehende zentrale Daten namensbasiert auf und orchestriert die Repository-Aufrufe. Beim Aktualisieren konstruiert er das neue unveränderliche Recipe mit der bestehenden UUID; das Repository ersetzt dessen gespeicherte Daten atomar. Mengen akzeptieren positive Ganz- und Dezimalzahlen mit Komma oder Punkt als Dezimaltrennzeichen und werden ohne `double` oder `float` direkt als `BigDecimal` verarbeitet. Zubereitungsschritte sind optional; nichtleere Einträge werden aus ihrer sichtbaren, lückenlos nummerierten Reihenfolge erzeugt und können später ergänzt werden. SQL, Portionierung und Suchlogik bleiben außerhalb der Controller.

`WeekPlanController` rendert die sieben vom `WeeklyMealPlanService` gelieferten Tage als responsive Karten. Er formatiert ausschließlich sichtbare deutsche Tages- und Datumsangaben, verwaltet Recipe-Auswahl und Spinner als UI-Zustand und delegiert Laden, Planen, Ersetzen und Entfernen vollständig an den Service. Geplante Gerichte öffnen über den bestehenden `ViewNavigator` dieselbe Recipe-Detailansicht wie die Gerichte- und Suchansichten. SQL und Kalenderberechnung bleiben außerhalb des Controllers; die Einkaufslistenansicht ist weiterhin ein Platzhalter.
